...
import { Search, UploadCloud, Mic, AlertCircle, Play, Square, Bookmark, Link2, Download } from "lucide-react";
          ...
          <div className="relative flex items-center p-2">
            ...
            <div className="flex items-center gap-1 pr-2">
              <button
                ...
                className={`p-2.5 rounded-xl transition-all duration-200 ${
                  file
                    ...
                    : isFocused
                      ? "text-zinc-400 hover:text-accent-amber hover:bg-accent-amber/10"
              ...
              <div className="relative">
                ...
                {isRecording && (
                  <span className="absolute -top-1.5 -right-1.5 text-[9px] font-mono font-bold text-red-400 bg-red-950 border border-red-800 rounded-full px-1.5 py-0.5 leading-none">
                ...
                <div className="flex flex-col gap-3 p-4">
                  <div className="flex items-center justify-center gap-1 mb-1">
                    {(["text", "upload", "mic"] as const).map((mode) => (
                      <span
                        key={mode}
                        className={`text-[10px] uppercase tracking-[0.15em] font-bold px-3 py-1 rounded-full transition-all duration-200 ${
                          activeMode === mode
                  ...
                  <div className="flex gap-3 w-full">
                    ...
                    >
                      <div className="absolute inset-0 bg-linear-to-b from-accent-amber/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                      <UploadCloud
                        size={36}
                    ...
                    >
                      <div className="absolute inset-0 bg-linear-to-b from-red-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                      {isRecording ? (
                  ...
                  {!hasInput && (
                    <p className="text-xs text-zinc-500 text-center font-medium tracking-wide">
                  ...
                  <button
                    ...
                    className={`w-full mt-1 py-4 rounded-xl transition-all duration-300 text-black font-bold tracking-widest uppercase group relative ${
                      hasInput
                        ? "bg-accent-amber hover:bg-accent-gold shadow-[0_0_20px_rgba(212,139,34,0.2)] hover:shadow-[0_0_35px_rgba(229,169,61,0.4)] cursor-pointer"
                        : "bg-accent-amber/30 cursor-not-allowed"
                    } disabled:opacity-50 disabled:cursor-not-allowed`}
                  >
                    <div className="flex items-center justify-center gap-2">
                      <span className={hasInput ? "group-hover:scale-105 transition-transform" : ""}>
        ...
        {results.length > 0 ? (
          ...
          >
            ...
            {results.map((result, i) => (
              <motion.div
                key={result.id}
                className="group/card flex flex-col gap-2 p-4 bg-[#111]/80 backdrop-blur-sm rounded-2xl border border-zinc-800 tactile-shadow hover:border-zinc-700 transition-colors"
                initial={{ opacity: 0, y: 15 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                ...
                {(result.bpm || result.tags || result.year) && (
                  <div className="flex items-center gap-1.5 px-2 flex-wrap">
                    {result.bpm && (
                      <span className="text-[10px] font-mono text-zinc-500">{result.bpm} BPM</span>
                    )}
                    {result.tags?.map((tag) => (
                      <span key={tag} className="text-[10px] font-mono text-zinc-600 bg-zinc-800/50 px-1.5 py-0.5 rounded">
                        {tag}
                      </span>
                    ))}
                    {result.year && (
                      <span className="text-[10px] font-mono text-zinc-500">{result.year}</span>
                    )}
                  </div>
                )}
                <AudioPlayer url={result.url} />
                <div className="flex items-center gap-2 px-2 opacity-0 group-hover/card:opacity-100 transition-opacity duration-200">
                  <button className="text-zinc-600 hover:text-accent-amber transition-colors" title="Save">
                    <Bookmark size={14} />
                  </button>
                  <button className="text-zinc-600 hover:text-accent-amber transition-colors" title="Copy link">
                    <Link2 size={14} />
                  </button>
                  <button className="text-zinc-600 hover:text-accent-amber transition-colors" title="Download">
                    <Download size={14} />
                  </button>
                </div>
              </motion.div>
