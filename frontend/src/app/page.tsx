...
/**
 * Render the main search UI for the Sonic Search Engine, handling text queries, file uploads, and microphone recording while presenting scan progress and results.
 *
 * The component manages local state for uploaded/recorded audio, query text, scanning status, results, errors, input focus, and recording timing. It produces a file preview URL for selected audio (and revokes it on change), may request microphone permission to record audio, and updates an ambient scanning visual while scans are in progress. The UI includes authentication controls, an expandable input panel with upload/record actions, a scanning overlay, and a results list with playback, save, share, and download actions.
 *
 * @returns The Home page React element containing the searchable interface, scanning overlay, authentication controls, and results display.
 */
